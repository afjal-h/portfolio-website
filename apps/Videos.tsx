...
const VideoApp: React.FC<VideoAppProps> = ({ onHome }) => {
  ...
  return (
    <div className="h-full w-full p-4 md:p-8 pb-32 overflow-y-auto">
      ...
      {activeVideo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-2 md:p-12 animate-[fade-in_0.2s_ease-out]">
            <div className="relative w-full max-w-5xl bg-black rounded-lg overflow-hidden shadow-2xl aspect-video flex flex-col">
                ...
                <div className="w-full h-full flex items-center justify-center bg-gray-900 text-white flex-col gap-4">
                     <p className="text-xl md:text-2xl font-bold px-4 text-center">{activeVideo.title}</p>
                     <div className="w-12 h-12 md:w-16 md:h-16 border-4 border-white/30 border-t-white rounded-full animate-spin"></div>
                     <p className="text-gray-400">Loading Embed...</p>
                     <p className="text-xs md:text-sm text-gray-500">(In a real app, this would be a YouTube/Vimeo iframe)</p>
                </div>
            </div>
        </div>
      )}
